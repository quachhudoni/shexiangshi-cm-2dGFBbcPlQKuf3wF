...
深度学习的目标是最小化一个高维、非凸的损失函数 L(θ)，其参数 θ 的维度可达成千上万甚至万亿。我们可以将这个损失函数想象成一个极其复杂、崎岖不平的“地貌景观”（Loss Landscape）。我们的任务，就是从一个随机的初始位置（参数初始化），一步步走到这个地貌的最低点（全局最小值）。

优化器就是我们的“寻路策略”。最朴素的想法是，在当前位置环顾四周，找到最陡峭的下坡方向，然后朝着这个方向走一小步。这个“最陡峭的下坡方向”就是**负梯度** (−∇θL(θ))。所有优化器都围绕着两个核心问题来构建策略：
...
L(θ)=1n∑i=1nℓ(fθ(xi),yi)

其中 n 是样本总数。要计算真实梯度 ∇θL(θ)，需要遍历所有 n 个样本。在动辄百万、千万甚至亿级样本的数据集上，计算一次真实梯度的成本是无法接受的。此外，神经网络的非凸性意味着，即使方向完全准确，也可能陷入局部最优或鞍点。因此，我们需要更高效、且具备一定“探索性”的梯度近似方法。

### **2. 公式推导与含义**

设当前参数为 θt，学习率为 η>0，单个样本 (xi,yi) 产生的梯度为 gi=∇θℓ(fθt(xi),yi)。
...
* **随机梯度下降 (Stochastic Gradient Descent, SGD)**
  ...
  **含义**：用**单个**随机样本的梯度作为整体梯度的无偏估计。E[git]=gt，但其方差巨大。每一步更新成本极低，但轨迹充满噪声，非常“摇晃”。
* **小批量梯度下降 (Mini-Batch Gradient Descent, MBGD)**
  随机抽取一个大小为 B 的小批量 Bt⊂{1,...,n}：
...
| 方法 | 优点 | 缺点 | 适用场景 |
| --- | --- | --- | --- |
| **BGD** | 更新方向稳定，理论性质好 | 计算成本极高；非凸场景易陷入鞍点 | 仅适用于小型数据集或凸优化问题 |
| **SGD** | 更新成本极低；噪声有助于跳出局部最优和鞍点 | 梯度方差大，收敛过程震荡剧烈，需要小心调整学习率 | 理论分析，或作为其他更复杂优化器的基础 |
| **MBGD** | 计算高效（可并行化），梯度方差可控 | 仍可能在病态曲率的“峡谷”地貌中震荡 | **深度学习的默认选择** |
...
* **经典动量 (Heavy-ball Momentum)**
  引入一个“速度”或“一阶动量”向量 vt，它是历史梯度的指数移动平均（Exponentially Moving Average, EMA）。
  ...
  + β 通常取 0.9 左右。
    **含义**：vt 累积了过去梯度。如果历史梯度方向一致，vt 会越来越大，实现加速；如果历史梯度方向反复变化，vt 中的正负项会相互抵消，实现减速。β 扮演了“摩擦系数”的角色，决定了历史梯度的衰减速度。
* **Nesterov 加速梯度 (Nesterov Accelerated Gradient, NAG)**
  ...
  1. **预估未来位置** (Lookahead position): θlookahead=θt−ηβvt−1
  2. **在未来位置计算梯度**: glookahead=∇θL(θlookahead)
  3. **更新速度与位置**:
...
动量方法统一地调整了所有参数的更新方向，但未解决另一个问题：**所有参数共享同一个学习率 η 是否合理？**
...
Adagrad 为每个参数维护一个累积的平方梯度值 st。

其中表示逐元素相乘其中表示逐元素相乘st=st−1+gt⊙gt(其中 ⊙ 表示逐元素相乘)

θt+1=θt−ηst+ϵ⊙gt

**含义**：

* st,j（st 的第 j 个分量）累加了参数 θj 自训练开始以来所有梯度的平方。
* 参数 θj 的**有效学习率**变成了 ηst,j+ϵ。
* 如果一个参数的梯度一直很大（更新频繁），st,j 会很大，其有效学习率就会变小。
* 如果一个参数的梯度一直很小或很稀疏（更新很少），st,j 会很小，其有效学习率就会较大。
* ϵ 是一个极小的常数（如 10−8），用于防止分母为零。
...
* **缺点**：
  + **致命缺陷**：分母中的 st 是单调递增的。随着训练的进行，所有参数的有效学习率最终都会趋近于零，导致训练提前“死亡”，无法继续学习。
...
* 与Adagrad唯一的区别在于 st 的计算方式。它不再是简单的累加，而是对历史平方梯度的加权平均。
* ρ 是一个衰减率（通常设为0.9或0.99），它控制了历史信息的遗忘速度。
* 这种机制使得 st 不再单调递增。如果近期梯度变小，st 也会随之下降，从而让有效学习率能够回升。这使得RMSProp能更好地适应非平稳目标（即损失函数的形状在训练过程中会发生变化）。
...
* **Adam**
  Adam同时维护梯度的一阶矩（动量）mt 和二阶矩（未开方的RMSProp项）vt 的指数移动平均：

  1. **更新一阶矩和二阶矩估计**:

     动量项动量项mt=β1mt−1+(1−β1)gt(动量项)

     自适应项自适应项vt=β2vt−1+(1−β2)gt⊙gt(自适应项)
  2. **偏差修正 (Bias Correction)**:
     由于 m0 和 v0 初始化为0，在训练初期，mt 和 vt 会偏向于0。Adam通过以下方式进行修正：
  ...
  **含义**：Adam的更新规则可以直观理解为：**用带有动量的梯度 m^t 作为更新方向，同时用类似RMSProp的 v^t 来对每个参数的学习率进行自适应缩放**。偏差修正确保了在训练初期，矩估计也是无偏的。
* **AdamW (Adam with Decoupled Weight Decay)**
  在标准的L2正则化中，损失函数会增加一项 λ2||θ||2，其梯度为 λθ。在Adam中，这个正则项梯度会进入 mt 和 vt 的计算，使得权重衰减的效果与梯度的量级耦合在一起，导致对大学习率和较大权重的参数，实际的权重衰减效果会减弱。
  AdamW 将权重衰减从梯度计算中解耦出来，直接在参数更新步骤中应用：

  1. **执行标准的Adam更新 (不含正则项的梯度)**：

     θt+1′=θt−η⋅AdamUpdate(m^t,v^t)
  2. **独立进行权重衰减**:
...
* **Adam/AdamW**: `betas` 通常设为 `(0.9, 0.999)`。对于训练非常长或梯度变化快的任务，可尝试减小 β2 (如 `0.98` 或 `0.95`) 以提高对近期梯度的响应速度。`eps` (ε) 通常为 `1e-8`。`weight_decay` (λ) 是一个关键的正则化超参，需根据模型和任务进行调整（典型值范围 `1e-4` 到 `1e-1`）。
